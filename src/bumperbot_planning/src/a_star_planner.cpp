...
namespace bumperbot_planning
{
            ...
            std::function<bool()> cancel_checker) 
  {
    ...
    while(!pending_nodes.empty() && rclcpp::ok()){ //exploration loop
      ...
      for(const auto & dir : explore_directions){ //explore neighbours of the graph node
        ...
        poseOnMap(new_node) && costmap_->getCost(new_node.x, new_node.y) < 99 && costmap_->getCost(new_node.x, new_node.y) >= 0){  // check it is already a visited node and check if it lies on the map
          new_node.cost = active_node.cost + 1 + costmap_->getCost(new_node.x, new_node.y); //edge cost =1
  ...
  double AStarPlanner::manhattanDistance(const GraphNode & node, const GraphNode goal_node) //i/p: current node +destination node
  {
