      ...
      //prev_time(rclcpp::Time(0))
{
    ...
    RCLCPP_INFO_STREAM(get_logger(), "NoisyController starting up...");
...
void NoisyController::jointCallback(const sensor_msgs::msg::JointState & msg)
{
    ...
    if (prev_time.seconds() == 0.0) {
        left_wheel_prev_position_ = wheel_encoder_left;
        right_wheel_prev_position_ = wheel_encoder_right;
        prev_time = msg_time;
        return;
    }

    double dp_left = wheel_encoder_left - left_wheel_prev_position_;
    double dp_right = wheel_encoder_right - right_wheel_prev_position_;
