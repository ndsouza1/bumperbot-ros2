    ...
    is_first_odom(true)
{
    ...
    if (nr_samples > 0){
        samples_.poses = std::vector<geometry_msgs::msg::Pose>(nr_samples, geometry_msgs::msg::Pose());
    }
    else {
...
void OdometryMotionModel::odomCallback(const nav_msgs::msg::Odometry & odom)
{
    ...
    double delta_trans = sqrt(std::pow(odom_y_increment,2)+std::pow(odom_x_increment, 2));
    double delta_rot2 = angle_diff(odom_theta_increment, delta_rot1);

    double rot1_variance = alpha1_ * delta_rot1 + alpha2_ * delta_rot2;
    ...
    for(auto & sample : samples_.poses){
        ...
        tf2::Quaternion samples_q(sample.orientation.x, sample.orientation.y, sample.orientation.z, sample.orientation.w);
        tf2::Matrix3x3 sample_m(samples_q);
        ...
        sample.position.y += delta_trans_draw * sin(sample_yaw + delta_rot2_draw);
        tf2::Quaternion q;
