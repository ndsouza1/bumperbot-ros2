    ...
    measurement_variance_(0.5)
{
    ...
    imu_sub_ = create_subscription<sensor_msgs::msg::Imu>("imu", 1000, std::bind(&KalmanFilter::imuCallback, this, _1));
    odom_pub_ = create_publisher<nav_msgs::msg::Odometry>("bumperbot_controller/odom_kalman", 10);
}

void KalmanFilter::measurementUpdate()
{
    double new_mean = (variance_ * imu_angular_z_ + measurement_variance_ * mean_) / (variance_ + measurement_variance_);
    double new_variance = (variance_ * measurement_variance_) / (variance_ + measurement_variance_);
    
    mean_ = new_mean;
    variance_ = new_variance;
}

void KalmanFilter::odomCallback(const nav_msgs::msg::Odometry & odom)
{
    RCLCPP_INFO(this->get_logger(), "Odom angular_z: %f, IMU angular_z: %f", 
    ...
    RCLCPP_INFO(this->get_logger(), 
           "Before KF - Odom: %f, IMU: %f, Mean: %f", 
           odom.twist.twist.angular.z, imu_angular_z_, mean_);

    measurementUpdate();

    RCLCPP_INFO(this->get_logger(), 
            "After KF - Mean: %f", mean_);
...
void KalmanFilter::imuCallback(const sensor_msgs::msg::Imu & imu)
{
    ...
    RCLCPP_INFO(this->get_logger(), "IMU angular_z: %f", imu_angular_z_);
}
